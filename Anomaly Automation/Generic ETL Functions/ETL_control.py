...
if anom.json_obj.has_key('Redshift'):
  ...
  RS_HOST = RS_HOST % REDSHIFT['host'];                                                                                       # redshift host from json file
...
if anom.json_obj.has_key('s3'):
  S3 = anom.json_obj['s3'];
  S3_BUCKET = "s3://%s" % S3['s3-bucket'];                                                                                                # S3 bucket name from json file
...
# Determine file type from file magic number
    ...
    for file in INPUT_FILES:
      anom.Ziplst = [];                                                                                                       # initialise zip file list
...
# Un-zip if necessary
      ...
      if (Filtyp == anom.ZIPEXT):
        try:
          anom.unzip_file(file);                                                                                              # gzip
          anom.wrt_log('unzipped %s' % file);
          anom.del_fil(file);                                                                                                 # delete .csv file
          file = anom.Ziplst[0];
        except:
          anom.wrt_log(anom.Errmsg);
          continue;
...
# Create .gz files for upload to S3
      ...
      if (len(anom.Ziplst) > 0) or (Filtyp in [anom.CSVEXT, anom.GZFEXT]):
        try:
...
# Put the files to S3
    ...
    anom.wrt_log('%d GZipped Files selected %s' % (len(S3_GZ_FILES), file_search));

    S3_GZ_FILES.sort();

    for file in S3_GZ_FILES:
      s3_cmd = 's3cmd %s put %s %s%s' % (magic, file, S3_BUCKET, s3_path);
