...
TIMEFORMAT1 = 'auto';
anom.curdat()
FILE_TIMESTAMP = (anom.datval - timedelta(days=2)).strftime("%m-%d-%Y");
FILE_TIMESTAMP_STD = (anom.datval - timedelta(days=2)).strftime("%Y-%m-%d");
#FILE_TIMESTAMP = "07-03-2015"
...
MATCH_FILE_LIST = \
[
  'NetworkMatchtablesIC_7921_' + FILE_TIMESTAMP + '_user_defined_spot_vars.log',
  'NetworkMatchtablesIC_7921_' + FILE_TIMESTAMP + '_states.log',
  'NetworkMatchtablesIC_7921_' + FILE_TIMESTAMP + '_page_flight_cost.log',
  'NetworkMatchtablesIC_7921_' + FILE_TIMESTAMP + '_page_cost.log',
  'NetworkMatchtablesIC_7921_' + FILE_TIMESTAMP + '_operating_systems.log',
  'NetworkMatchtablesIC_7921_' + FILE_TIMESTAMP + '_met_event.log',
  'NetworkMatchtablesIC_7921_' + FILE_TIMESTAMP + '_keyval.log',
  'NetworkMatchtablesIC_7921_' + FILE_TIMESTAMP + '_designated_market_area.log',
  'NetworkMatchtablesIC_7921_' + FILE_TIMESTAMP + '_custom_creative_fields.log',
  'NetworkMatchtablesIC_7921_' + FILE_TIMESTAMP + '_creative_ad_assignments.log',
  'NetworkMatchtablesIC_7921_' + FILE_TIMESTAMP + '_countries.log',
  'NetworkMatchtablesIC_7921_' + FILE_TIMESTAMP + '_cities.log',
  'NetworkMatchtablesIC_7921_' + FILE_TIMESTAMP + '_campaign.log',
  'NetworkMatchtablesIC_7921_' + FILE_TIMESTAMP + '_dartsearch.log',
  'NetworkMatchtablesIC_7921_' + FILE_TIMESTAMP + '_activity_type.log',
  'NetworkMatchtablesIC_7921_' + FILE_TIMESTAMP + '_creative.log',
  'NetworkMatchtablesIC_7921_' + FILE_TIMESTAMP + '_page.log',
  'NetworkMatchtablesIC_7921_' + FILE_TIMESTAMP + '_site.log',
  'NetworkMatchtablesIC_7921_' + FILE_TIMESTAMP + '_activity_cat.log',
  'NetworkMatchtablesIC_7921_' + FILE_TIMESTAMP + '_ad.log',
  'NetworkMatchtablesIC_7921_' + FILE_TIMESTAMP + '_ad_page_assignments.log',
  'NetworkMatchtablesIC_7921_' + FILE_TIMESTAMP + '_advertiser.log',
  'NetworkMatchtablesIC_7921_' + FILE_TIMESTAMP + '_browsers.log',
];
...
MATCH_TABLE_LIST = \
[
  'dfa_user_defined_spot_vars',
  'dfa_states',
  'dfa_page_flight_cost',
  'dfa_page_cost',
  'dfa_operating_systems',
  'dfa_met_event',
  'dfa_keyval',
  'dfa_designated_market_area',
  'dfa_custom_creative_fields',
  'dfa_creative_ad_assignments',
  'dfa_countries',
  'dfa_cities',
  'dfa_campaign',
  'dfa_dartsearch',
...
# build where clause in case there are mutiple primary keys
  ...
  with conn:
    with conn.cursor() as cursor:
      ...
      for row in rows:
        if i == 0:
          where_str += SCHEMA_NAME + '{0}' + "." + row[2] + "=" + SCHEMA_NAME + STAGING + '{0}' + "." + row[2]
        else:
          where_str += " and " + SCHEMA_NAME + '{0}' + "." + row[2] + "=" + SCHEMA_NAME + STAGING + '{0}' + "." + row[2]
        i += 1
  return where_str
...
def staging_sql():
  ...
  [
    [
     "BEGIN TRANSACTION;",
     "INSERT INTO %s%s SELECT TRUNC(starttime),*,'%s' " % (SCHEMA_NAME, '{0}', FILE_TIMESTAMP_STD) +\
     "FROM %s%s%s WHERE advertiserid IN (%s);" % (SCHEMA_NAME, STAGING, '{0}', ','.join(anom.input_lists[0])),
     "END TRANSACTION;"
    ],
  ...
  for t in MATCH_TABLE_LIST:
    ...
    sql_string = "DELETE FROM %s%s " % (SCHEMA_NAME, '{0}') + \
    "USING %s%s%s WHERE %s;" %  (SCHEMA_NAME, STAGING, '{0}',  build_sql_string(t));
    SQL_LIST_SMALL.append(sql_string)
    SQL_LIST_SMALL.append("INSERT INTO %s%s SELECT * FROM %s%s%s;" % (SCHEMA_NAME, '{0}', SCHEMA_NAME, STAGING,'{0}'))
...
TO_ADDR = ['leo.li@anom.com.au','tony.edward@anom.com'];
