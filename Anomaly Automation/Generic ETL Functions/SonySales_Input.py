...
def staging_sql():
  ...
  [
    [
      ...
      "WHERE " + \
      "sale_date >= (SELECT MIN(sale_date) FROM %s%s%s) " % (SCHEMA_NAME, STAGING, '{0}') + \
      "AND " + \
      "sale_date <= (SELECT MAX(sale_date) FROM %s%s%s);" % (SCHEMA_NAME, STAGING, '{0}'),
      "INSERT " + \
...
TO_ADDR = ['tony.edward@anom.com.au', 'leo.li@anom.com.au', 'andrew.yap@sonymusic.com'];
